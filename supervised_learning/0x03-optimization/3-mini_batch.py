    ...
    with tf.Session() as session:
        ...
        for j in range(epochs + 1):
            ...
            for step_number in range(0, int(m/32)):
                ...
                step_accuracy = session.run(accuracy, feed_dict={x: X_batch, y: Y_batch})
                step_cost = session.run(loss, feed_dict={x: X_batch, y: Y_batch})
                if (counter == 100):
            ...
            step_accuracy = session.run(accuracy, feed_dict={x: X_resto, y: Y_resto})
            step_cost = session.run(loss, feed_dict={x: X_resto, y: Y_resto})
            session.run(train_op, feed_dict={x: X_resto, y: Y_resto})
