...
def inception_network():
    ...
    l2_1 = K.layers.Conv2D(filters=64, kernel_size=1,
                           padding='same',
                           activation=activation,
                           kernel_initializer=k_init)(l_pool1)

    l2 = K.layers.Conv2D(filters=192, kernel_size=3,
                         padding='same',
                         activation=activation,
                         kernel_initializer=k_init)(l2_1)

    l_pool2 = K.layers.MaxPooling2D(pool_size=[3, 3],
                                    strides=2,
                                    padding='same')(l2)
    ...
    l_pool3 = K.layers.MaxPooling2D(pool_size=[3, 3],
                                    strides=2,
                                    padding='same')(l4)
    ...
    l_pool4 = K.layers.MaxPooling2D(pool_size=[3, 3],
                                    strides=2,
                                    padding='same')(l9)
    ...
    Y = K.layers.Dense(1000, activation='softmax',
                       kernel_initializer=k_init)(dropout)
