...
def pool_backward(dA, A_prev, kernel_shape, stride=(1, 1), mode='max'):
    ...
    kh = kernel_shape[0]
    kw = kernel_shape[1]
    x_patches = A_prev.reshape(A_prev.shape[0],A_prev.shape[1]//2, kh,A_prev.shape[2]//2, kw,A_prev.shape[3])
    if mode is 'max':
        out = x_patches.max(axis=1).max(axis=2)
        mask  =np.isclose(A_prev,np.repeat(np.repeat(out,kh,axis=1),kw,axis=2)).astype(int)
