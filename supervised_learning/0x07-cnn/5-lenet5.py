#!/usr/bin/python3
"""
Write a function def lenet5(X): that builds a modified version of the LeNet-5 architecture
using keras:
...
Returns: a K.Model compiled to use Adam optimization (with default hyperparameters) and
accuracy metrics
"""
...
def lenet5(x):
    """Function lenet5"""
    model = K.Sequential()
    model.add(K.layers.Conv2D(filters=6, kernel_size=(5,5), padding='same', activation='relu'))
    model.add(K.layers.MaxPooling2D(pool_size=(2, 2), strides=(2,2)))
    model.add(K.layers.Conv2D(filters=16, kernel_size=(5, 5), padding='valid', activation='relu'))
    model.add(K.layers.MaxPooling2D(pool_size=(2, 2), strides=(2,2)))
